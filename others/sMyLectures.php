...
<h2><i class="glyphicon glyphicon-th"></i>My Lectures</h2>
...
<tbody>
  ...
  $timeTable = $database->getDataList("select *from  uplaodlecture  where semester='$semester' AND batch='$batch' AND section='$sectionName' "
          . "AND program='$programId'");
  
  if($timeTable){
      while($timeTableRow =$timeTable->fetch_assoc()){?>
    <tr>
        <td><?php echo $timeTableRow['title']; ?></td>
        <?php
      ...
      ?>
       ...
       <td><a href="Downloadlecture.php?id=<?php echo $timeTableRow['id']; ?>" > Download</a>  </td>
